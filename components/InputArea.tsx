...
import { stopAudio, unlockAudioContext } from "../services/geminiService";
...
const InputArea: React.FC<InputAreaProps> = ({ onSend, disabled }) => {
  ...
  const handleSend = async () => {
    if (inputText.trim()) {
      await unlockAudioContext(); // Unlock audio
  ...
  const toggleListening = async () => {
    ...
    if (isListening) {
      ...
      await unlockAudioContext(); // Unlock audio immediately on mic click
